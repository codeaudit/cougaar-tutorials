...
public final class FractalMath {
    ...
    * @param cx for the Mandlebrot Set use x0, otherwise for the Julia Set use a
    *        constant (e.g. -0.70176)
    * @param cy for the Mandlebrot Set use y0, otherwise for the Julia Set use a
    *        constant (e.g. -0.3842)
    */
    ...
    *        Julia Set, otherwise ignore the "cx" and "cy" values and compute
    *        the MandelbrotSet
    ...
    */
   public static byte[] compute(int width, int height, double x_min, double x_max, double y_min, double y_max, boolean isJulia,
                                double cx, double cy) {
      ...
      compute(width, height, x_min, x_max, y_min, y_max, isJulia, cx, cy, data);
    ...
    */
   public static void compute(int width, int height, double x_min, double x_max, double y_min, double y_max, boolean julia,
                              double cx, double cy, byte[] data) {

      // validate
      validateRange(width, height);
      int data_length = (height * width);
      if (data == null || data.length < data_length) {
         throw new IllegalArgumentException("Given " + (data == null ? "null" : "small byte[" + data.length + "]")
