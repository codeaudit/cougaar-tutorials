...
public class ScheduleServlet
   ...
   private void execute(HttpServletRequest request, HttpServletResponse response)
      ...
      for (int i = 0; i < failedAllocs.size(); i++) {
         ...
         out.println("<tr><td>" + alloc.getTask().getVerb() + " "
               + alloc.getTask().getDirectObject().getItemIdentificationPG().getItemIdentification() + "</td></tr>");
      }
      out.println("</table>");
    ...
    */
   private void dumpProgrammerSchedule(ProgrammerAsset pa, PrintWriter out, ArrayList failedAllocs) {
      ...
      while (iter.hasMoreElements()) {
         ...
         if (o instanceof Allocation) {
            ...
            String monthStr = startStr.equals(endStr) ? startStr : (startStr + "-" + endStr);
            if (alloc.getEstimatedResult().isSuccess()) {
               out.print("<tr><td>" + monthStr + "</td><td>" + alloc.getTask().getVerb() + " "
                     + alloc.getTask().getDirectObject().getItemIdentificationPG().getItemIdentification() + "</td></tr>");
            } else {
