...
/**
 * This COUGAAR Plugin expands tasks of verb "CODE" into workflows of subtasks:
 * DESIGN DEVELOP TEST
 * 
 * @author ALPINE (alpine-software@bbn.com)
 * @version $Id: DevelopmentExpanderPlugin.java,v 1.3 2004-01-21 17:25:50 jwong
 *          Exp $
 **/
public class DevelopmentExpanderPlugin
   ...
   public void setupSubscriptions() {
      allCodeTasks = (IncrementalSubscription) getBlackboardService().subscribe(allCodeTasksPredicate);
   }
   ...
   private void setPreferences(NewTask new_task, long start, int duration, long deadline) {
      ...
      ScoringFunction scorefcn = ScoringFunction.createStrictlyAtValue(AspectValue.newAspectValue(AspectType.START_TIME, start));
      Preference pref = ((PlanningFactory) domainService.getFactory("planning")).newPreference(AspectType.START_TIME, scorefcn);
      preferences.add(pref);

      scorefcn = ScoringFunction.createStrictlyAtValue(AspectValue.newAspectValue(AspectType.END_TIME, deadline));
      pref = ((PlanningFactory) domainService.getFactory("planning")).newPreference(AspectType.END_TIME, scorefcn);
      preferences.add(pref);

      scorefcn = ScoringFunction.createStrictlyAtValue(AspectValue.newAspectValue(AspectType.DURATION, duration));
