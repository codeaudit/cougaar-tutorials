 ...
 * both in time and in accuracy. But the Black router is not be available to 
 * a real fielded application, because of the HAIPE security boundary.
  * <p>
 * This datafeed only works in the KNODE. 
 * It assumes a special access port was created to get around the HAIPE and 
 * access the Black router. Also, the HAIPE address translation is hard coded in the map 
 * {@link HaipeRedToBlackAddressTranslator}
 * <p>
 * Extra steps are need to map from the Red-side neighbor to the black WNW neighbor
 * <ol>
