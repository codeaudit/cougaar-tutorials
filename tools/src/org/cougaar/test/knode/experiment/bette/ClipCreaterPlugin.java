...
public class ClipCreaterPlugin
   ...
   private class captureNextImageRunnable
      ...
      public void run() {
         if (captureState == null || !captureState.getCurrentState().equals(ClipCaptureState.StateKind.Grabbing)) {
         ...
         if (image.length > 0) {
            ImageHolder imageHolder = new ImageHolder(uids, expectedCaptureTime, image, clipName, count, clip.getClipId());
         ...
         if (triggerDelayMillis <= 0) {
            ...
            if (log.isInfoEnabled()) {
               log.info(clipName + " Missed " + numberOfMissedCaptures + " captures, Skipping ahead " + (triggerDelayMillis)
                     + " millis" + " count=" + count);
            }
         ...
         if (log.isInfoEnabled()) {
            log.info("Capture late by " + (now - expectedCaptureTime) + " millis." + " Triggering in " + triggerDelayMillis
                  + " millis. " + " Capture Delay " + (now - startTime));
         }
         Runnable sendNext = new captureNextImageRunnable(uids, clip, nextCaptureTime, waitTimeMillis, count);
         captureNextSchedulable =
               threadService.getThread(ClipCreaterPlugin.this, sendNext, "VideoCapture", ThreadService.WILL_BLOCK_LANE);
   ...
   public boolean isMyStartCapture(ClipCaptureState state) {
      ...
      return outstandingCommand != null && outstandingCommand.equals(ClipCaptureState.CommandKind.StartCapture);
   }
   ...
   public boolean isMyStopCapture(ClipCaptureState state) {
      ...
      CommandKind outstandingCommand = state.getOutstandingCommand();
      return outstandingCommand != null && outstandingCommand.equals(ClipCaptureState.CommandKind.StopCapture);
   }
   ...
   public boolean isMySendClip(ClipCaptureState state) {
      ...
      CommandKind outstandingCommand = state.getOutstandingCommand();
      return outstandingCommand != null && outstandingCommand.equals(ClipCaptureState.CommandKind.Send);
   }
   ...
   public boolean isMyClearClip(ClipCaptureState state) {
      ...
      CommandKind outstandingCommand = state.getOutstandingCommand();
      return outstandingCommand != null && outstandingCommand.equals(ClipCaptureState.CommandKind.Clear);
   }
}
