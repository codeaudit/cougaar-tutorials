...
public class ClipDisplayPlugin
   ...
   public void executeNewClip(ClipHolder clip) {
      ...
      if (displayingClip == null || displayingClip.getStartTime() < clip.getStartTime()) {
   ...
   private void scheduleNextDisplay(long waitTime, long nextLoopTime, ClipHolder clip) {
      ...
      displayNextSchedulable =
            threadService.getThread(ClipDisplayPlugin.this, displayNext, "DisplayImageFromClip", ThreadService.BEST_EFFORT_LANE);
   ...
   public boolean isMyClipNotDone(ClipHolder clip) {
      return clip.equals(displayingClip) && !clip.isDone();
   }

   public boolean isMyClipDone(ClipHolder clip) {
      return clip.equals(displayingClip) && clip.isDone() && !clip.isSend();
   }
