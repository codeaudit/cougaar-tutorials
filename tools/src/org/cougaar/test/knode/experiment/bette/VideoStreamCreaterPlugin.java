...
public class VideoStreamCreaterPlugin
   ...
   @Cougaar.Arg(name = "streamName", defaultValue = "DefaultStream", description = "Name of the Stream")
   public String streamName;

   @Cougaar.Arg(name = "waitForIncarnationNumber", defaultValue = "false", description = "Wait to send video, until agent is assigned and IncarnationNumber")
   ...
   private class sendNextImageRunnable
      ...
      public void run() {
         ...
         if (image.length > 0) {
            ImageHolder imageHolder = new ImageHolder(uids, expectedCaptureTime, image, streamName, count, myIncarnation);
         ...
         if (triggerDelayMillis <= 0) {
            ...
            if (log.isInfoEnabled()) {
               log.info(streamName + " Missed " + numberOfMissedCaptures + " captures, Skipping ahead " + (triggerDelayMillis)
                     + " millis" + " count=" + count);
            }
         ...
         sendNextSchedulable =
               threadService.getThread(VideoStreamCreaterPlugin.this, sendNext, "VideoCapture", ThreadService.WILL_BLOCK_LANE);
