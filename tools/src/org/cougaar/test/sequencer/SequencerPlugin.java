...
abstract public class SequencerPlugin<S extends Step, R extends Report, C extends Context>
    ...
    @Cougaar.Arg(name = "defaultWorkerTimeout", defaultValue = "10000", 
                 description = "Default time Aggregators should wait for all its Workers to complete a Step")
    public int defaultWorkerTimeout;

    @Cougaar.Arg(name = "defaultNodeTimeout", defaultValue = "60000", 
                 description = "Default time Sequencer should wait for all its Nodes to complete a Step")
    public int defaultNodeTimeout;

    
    
    private Map<String, NodeRegistrationEvent> registeredNodes = 
        new HashMap<String, NodeRegistrationEvent>();
    ...
    public void executeRegisterNode(NodeRegistrationEvent event) {
        ...
        if (registeredNodes.size() == nodeCount) {
            SocietyRegistrationEvent societyRegistrationEvent = 
                new SocietyRegistrationEvent(uids, numberOfWorkers, nodeCount, true);
    ...
    public void executeNodeCompletionEvent(NodeCompletionEvent<S, R> event) {
        ...
        if (processingStep != step) {
            // Wrong Step
            log.warn("Node " + nodeId + " reported during wrong step. Ignoring Report:" +
                     " currentStep=" +processingStep+ " reportedStep=" + step +
                     " success=" +successfulEvent );
            return;
        }
        if (completionReports.containsKey(nodeId)) {
            // Duplicate Report
            log.warn("Node " + nodeId + " reported multiple times. Ignoring Report:" +
                     " currentStep=" +processingStep+ " reportedStep=" + step +
                     " success=" +successfulEvent );
            return;
        }
        ...
        if (!successfulEvent) {
            ...
            SocietyCompletionEvent<S, R> completionEvent =
                    new SocietyCompletionEvent<S, R>(uids, step, completionReports, false);
        ...
        if (completionReports.size() == nodeCount && !failedDuringStep) {
            ...
            cancelNodeTimout();
            SocietyCompletionEvent<S, R> completionEvent =
                    new SocietyCompletionEvent<S, R>(uids, step, completionReports, true);
            completionReports = new HashMap<String, Set<R>>();
            blackboard.publishAdd(completionEvent);
            processingStep=null;
            return;
    ...
    private final class NodeTimeout implements Runnable {
